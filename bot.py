...
async def process_command(interaction, processor_func, *args, output_type="image"):
    ...
    try:
        ...
        titles = {"image": "Preview", "video": "360° Video", "mesh": "File", "pointcloud": "File"}
...
@bot.tree.command(name="dot", description="Convert mesh to point cloud")
...
@bot.tree.command(name="poly", description="Simplify mesh")
@app_commands.describe(simplify="Simplification factor (4-100)", output_type="Output: image/mesh/video")
...
async def upload_command(interaction: discord.Interaction, mesh_file: discord.Attachment):
    ...
    temp_path = os.path.join(tempfile.gettempdir(), f"mesh_{interaction.user.id}.obj")
